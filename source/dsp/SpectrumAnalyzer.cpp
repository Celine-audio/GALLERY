...
SpectrumAnalyzer::SpectrumAnalyzer (int fftOrderToUse)
    : fft (fftOrderToUse),
      ...
      numBins (fftSize / 2 + 1),
      // An eighth of the window rather than a half.
      //
      // The overlap is the display's frame rate: a frame cannot be measured until
      // enough new samples have arrived to advance the window, so at 50% overlap an
      // 8192-point transform at 48 kHz produces **eleven frames a second** however
      // fast the editor repaints. That is what "the analyser looks low fps" is. At an
      // eighth it is forty-seven, which is past what anybody can see stepping, and the
      // cost is four transforms where there was one -- paid only while somebody has
      // the trace switched on.
      hopSize (fftSize / 8),
